...
enum {
	MSM_TS_DEBUG_REGS = 1,
	MSM_TS_DEBUG_COORD = 1 << 1,
	MSM_TS_DEBUG_VKEY = 1 << 2,
};
...
static irqreturn_t msm_ts_irq(int irq, void *dev_id)
{
	...
	if (!was_down && down) {
		...
		if (vkey) {
			...
			if(msm_tsdebug & MSM_TS_DEBUG_VKEY) {
				dev_dbg(ts->dev, "%s: virtual key down %d\n", __func__,
				vkey->key);
			}
			ts->vkey_down = vkey;
		...
		if (!down) {
			if(msm_tsdebug & MSM_TS_DEBUG_VKEY) {
				dev_dbg(ts->dev, "%s: virtual key up %d\n", __func__,
				ts->vkey_down->key);
			}
...
#ifdef CONFIG_PM
static int msm_ts_suspend(struct device *dev)
{
	struct msm_ts *ts =  dev_get_drvdata(dev);

	if (device_may_wakeup(dev) && device_may_wakeup(dev->parent)) {
...
static int msm_ts_resume(struct device *dev)
{
	struct msm_ts *ts =  dev_get_drvdata(dev);

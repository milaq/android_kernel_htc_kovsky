...
//	return;
	if (on) {
		...
		printk("%s: enabled mddi bridge power\n", __func__);
		printk("%s: -powering up panel\n", __func__);
	} else {

		printk("%s: +shutting down panel\n", __func__);
